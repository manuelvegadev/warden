...
- [x] Block textures: `scripts/mc-atlas.mjs` derives a tile strip and a per-block face table from the fetched art; the terrain samples a texture array per face, tinted, occluded and shaded by vertex colour, with the flat colours as the fallback; leaves in the game's fancy (cut-outs with the leaves behind showing) or fast (solid canopies) graphics, a Video-tab setting; a pixel-scale setting (1×, the device's, 0.5×); a Quality slider (Potato: flat colours · Good: textures, solid leaves · Fancy: see-through leaves, device pixels) over the individual knobs; a screenshot button and F2 (done 2026-09-03). Non-cube blocks still draw as cubes with their particle texture
- [x] Blocks turn as in the game: the agent reads the block data of logs, pillars, furnaces, barrels, observers and the like and sends an orientation byte (`WCK3`); the atlas derives, per blockstate variant, the faces and texture turns the game's model rotation gives, and the mesher picks them (done 2026-09-03)
- [x] The server's light in the viewer: sky and block light per cell (`WCK4`), smooth lighting per vertex, the game's light curve and colours in the shader, torches lit at night, caves dark; a Night vision switch lights everything as daylight (done 2026-09-03). Replaces the canopy-depth heuristic
- [x] The rest of the game's block look: blocks whose blockstate lists several models (grass, dirt, stone, sand, netherrack) draw the one the game draws at that position, seeded with `Mth.getSeed`, so a plain does not repeat one tile; a water surface is 8/9 of a block tall as the game draws a source fluid; a block with no body of its own whose cell holds water (seagrass and kelp always, a coral fan or a sea pickle when waterlogged) is sent with a flag and drawn as that water, so the sea has no holes where a plant stands; a Brightness setting (the game's option, 45 % here) and a Map relief switch (done 2026-09-03)
...
- [x] Simple Voice Chat installed by the daemon (2026-09-04): the first start of a server that loads Bukkit plugins fetches the plugin from the catalog into `plugins/`, so voice works without anyone going to look for it; the release listed for the server's Minecraft version, or the newest one when the catalog lists none; a failure costs voice, not the start; removing it from the Plugins tab is remembered
