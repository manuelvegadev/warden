...
| POST | `/instances/{id}/map/{world}/chunks` | `{"chunks":[[cx,cz],…]}` (≤1024) → `application/octet-stream`: for each chunk the daemon holds, `i32 cx · i32 cz · u64 hash · u32 len · gzip payload` (little-endian). Unknown chunks are omitted. The payload format is the agent's `WCK4` (ADR-018; the daemon also holds older `WCK2`/`WCK3` chunks until they are re-sent). |
