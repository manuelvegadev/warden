...
## Decisión
- **Un binario `wardend`** que contiene: supervisor de instancias, API, WebSocket y UI.
- Cada instancia es un directorio (`/var/lib/warden/servers/<id>/`) con un `instance.json` (jar, versión, flags JVM, memoria, puerto, autostart) y el proceso `java` se lanza con `os/exec`, como proceso hijo del daemon.
...
- Instalación: unidad `systemd` `warden.service`.
