...
**Upgrading.** Once installed, Beacon's Home shows a card when a newer release exists and an admin can update from there (ADR-016: the daemon stages and verifies the binary, a root systemd unit installs it and restarts the service). The install script does the same from a shell: `curl -fsSL https://warden.manuelvega.dev/install.sh | sudo bash -s -- --yes` keeps the existing configuration (including a custom `WARDEND_LISTEN`).
