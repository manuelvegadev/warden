...
## 1. El problema
El panel (Next.js en Docker, dominio `panel.ejemplo.com`) y el daemon (Go, `wardend.ejemplo.com` o `:8080` en el host) viven en **orígenes distintos**. Hay que decidir: quién guarda los usuarios, dónde vive la credencial en el navegador, cómo se autentica el WebSocket y cómo se protege el daemon, que tiene poder total sobre procesos y archivos del host.
...
3. **WebSocket**: el cliente pide `POST /api/ws-ticket` al panel → el panel pide al daemon `POST /auth/ws-ticket` (con el JWT del usuario) → el daemon devuelve un **ticket opaco aleatorio de un solo uso, 30 s de vida**, ligado al usuario. El navegador abre `wss://wardend…/api/v1/ws` y envía `{"type":"auth","ticket":"…"}` como **primer mensaje**. El daemon valida `Origin` contra `WARDEND_ALLOWED_ORIGINS`, canjea el ticket (borrado atómico) y cierra si no hay auth en 5 s. Reconexión = nuevo ticket.
...
5. **Panel ↔ daemon (servidor a servidor)**: además del JWT del usuario, el panel se identifica con `X-Panel-Key: <secreto compartido>` (`WARDEND_PANEL_KEY`). Sin esa cabecera, el daemon rechaza `/auth/login` y `/auth/ws-ticket`. Así, aunque el daemon esté en Internet, solo el panel puede iniciar sesiones. Opcional: **mTLS** entre ambos si están en hosts distintos.
6. **Transporte**: TLS obligatorio fuera de `localhost`. Recomendado: daemon en `127.0.0.1:8080` y Traefik de Dokploy (mismo host) o Caddy como reverse proxy con HTTPS; alternativa: `WARDEND_TLS_CERT/KEY` para TLS nativo. HSTS.
...
8. **API tokens** (para scripts/CI): `Authorization: Bearer wd_<random>` creados en Ajustes, con `aud: "api"`, revocables, hash en DB.
...
- [ ] docs: guía de despliegue con Traefik/Dokploy y variables (`WARDEND_PANEL_KEY`, `WARDEND_ALLOWED_ORIGINS`, `PANEL_SESSION_SECRET`, `WARDEND_URL` interna vs `NEXT_PUBLIC_WARDEND_WS_URL` pública)

## 7. Actualización (ADR-009)
Se adopta **Better Auth** en el panel: la identidad pasa al panel (SQLite/Postgres), el daemon verifica JWT **offline vía JWKS** (`/api/auth/jwks`) con `aud="wardend"`, y el WS usa el JWT corto de `/api/auth/token` como primer mensaje en lugar del ticket opaco. Todo lo demás de §5 (BFF, cookie `HttpOnly`, `X-Panel-Key`, `Origin`, TLS, hardening) sigue vigente.
