...
| **Landing** (site) | [`landing/`](landing/) | Astro (static) + React islands, shares [`packages/ui`](packages/ui/) with Beacon | GitHub Pages — https://warden.manuelvega.dev |
...
```bash
curl -fsSL https://warden.manuelvega.dev/install.sh | sudo bash        # interactive; offers to run the Beacon panel with Docker
```
The script ([`landing/public/install.sh`](landing/public/install.sh)) downloads the latest release for your CPU, verifies it against `SHA256SUMS` and runs `wardend install`. Re-run it to upgrade (`… | sudo bash -s -- --yes` keeps the configuration). Without the script: download `wardend-linux-{amd64,arm64}` from [Releases](https://github.com/manuelvegadev/warden/releases), `chmod +x wardend && sudo ./wardend install`. Details, TLS modes and Dokploy in [`docs/deploy.md`](docs/deploy.md).
...
# Beacon (panel)
pnpm install                                 # once, at the root (pnpm workspace: beacon, landing, packages/ui); also fetches the game art for the live view into beacon/data/mc-assets and derives the block texture atlas (pnpm mc:assets, pnpm mc:atlas)
