...
/**
 * Reads every online player's position on the main thread (microseconds) and hands the JSON to the
 * client. Runs every 4 ticks. When the last player leaves one empty list is sent, then nothing.
 */
public final class PlayerSampler {
    ...
    public void tick() {
        ...
        client.sendText(encode(players).toString());
    }

    static JsonObject encode(Collection<? extends Player> players) {
